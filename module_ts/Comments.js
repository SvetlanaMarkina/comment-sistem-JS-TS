...
class Comments extends CommentSystem {
    constructor(userForm) {
        super();
        this.commentID = super.getNumberComments();
        this.rating = new Rating();
        this.favorites = new Favorite();
        this.replyes = new Answer(userForm, this.rating, this.favorites);
        this.filter = new Dropdown(this);
        this.updateComments();
        ...
        if (userForm.sendBtn)
            userForm.sendBtn.addEventListener("click", sendListener, false);
    }
    ...
    renderComment(html) {
        const comments = document.querySelector('.commentSystem__comments');
        if (comments) {
            comments.insertAdjacentHTML("afterbegin", html);
        }
    }
    hiddenComments(bool) {
        const comments = document.querySelectorAll('.commentSystem__commentBlock');
    ...
    updateComments() {
        const comments = document.querySelector('.commentSystem__comments');
        if (comments) {
            const commentBlocks = comments.querySelectorAll('.commentSystem__commentBlock');
        ...
        currentData.history.forEach((item) => {
            htmlTemplateComment = this.getTemplateComment(item.commentID, item.comment.commentNickname, item.comment.commentAvaPath, item.comment.commentText, item.comment.commentTime.displayDate);
    ...
    getTemplateComment(commentID, userNickname, userAva, commentsTxt, date) {
        return `
            ...
            <div class="comments__comment">
                ...
                <div class="comments__info">
                    ...
                    <div class="comments__buttons">
                        <button class="comments__btnAnswer">
