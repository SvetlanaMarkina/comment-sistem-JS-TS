...
class Favorite extends CommentSystem {
    ...
    cleanFavoriteComments() {
        const comments = document.querySelector('.commentSystem__comments');
        const favoriteBlocks = document.querySelectorAll('.commentSystem__commentBlock[data-favorite="favorite"]');
        if (comments)
            favoriteBlocks.forEach(item => comments.removeChild(item));
    }
    addListenerCommentsFavoritesBtns(commentID) {
        const commentBlockEl = document.querySelector(`[data-commentid="${commentID}"]`);
        if (commentBlockEl) {
            const favoritesBtn = commentBlockEl.querySelector('.comments__btnSelected');
            if (favoritesBtn)
                this.listenerFavoritesBtn(favoritesBtn, commentID);
        }
    }
    addListenerReplyFavoritesBtns(commentID, replyID) {
        const commentBlockEl = document.querySelector(`[data-commentid="${commentID}"]`);
        if (commentBlockEl) {
            const replyEl = commentBlockEl.querySelector(`[data-replyid="${replyID}"]`);
            if (replyEl) {
                const favoritesBtn = replyEl.querySelector('.comments__btnSelected');
                if (favoritesBtn)
                    this.listenerFavoritesBtn(favoritesBtn, commentID, replyID);
            }
        }
    }
    listenerFavoritesBtn(favoritesBtn, commentID, replyID) {
    ...
    updateFavoriteBtns(favoritesBtn, commentID, replyID) {
        const currentData = super.getDATA();
        if (replyID === undefined) {
            for (let favoriteComment in currentData.user.favorites) {
                if (+currentData.user.favorites[favoriteComment].srcCommentID === commentID
    ...
    getTemplateFavoriteComment(userNickname, userAva, commentsTxt, date) {
        return `
