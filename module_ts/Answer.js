"use strict";
class Answer extends CommentSystem {
    constructor(userForm, rating, favorites) {
        super();
    ...
    addListenerReplyBtn(commentID) {
        const commentBlock = document.querySelector(`[data-commentid="${commentID}"]`);
        if (commentBlock) {
            const replyBtn = commentBlock.querySelector('.comments__btnAnswer');
            const btnReplyListener = (event) => {
                var _a;
                if (event.target === replyBtn) {
                    ...
                    if (this.userForm.sendBtn)
                        this.userForm.sendBtn.dataset.mode = 'reply';
                    if (commentBlock.dataset.commentid)
                        this.commentID = +commentBlock.dataset.commentid;
                    this.preNickname = (_a = commentBlock.querySelector('.comments__name')) === null || _a === void 0 ? void 0 : _a.innerHTML;
                    ...
                    if (this.userForm.sendBtn)
                        this.userForm.sendBtn.dataset.mode = 'comment';
                }
            };
            if (commentBlock)
                commentBlock.addEventListener('click', btnReplyListener);
        }
    }
    ...
    getTemplateReply(replyID, userNickname, preNickname, userAva, replyTxt, date) {
        return `
        <div class="commentBlock__reply" data-replyid=${replyID}>
            ...
            <div class="comments__info">
                <div class="comments__title">
                    <h3 class="comments__name">${userNickname}</h3>
                    <h3 class="comments__prenickname">
                    ...
                    <time class="comments__time">${date}</time>
                </div>
                <p class="comments__text">
                    ${replyTxt}
                </p>
                <div class="comments__buttons">
                    <button class="comments__btnSelected" data-favorite="false">
    ...
    updateReply(commentBlock) {
        ...
        for (replyBlock in commentBlock.replyes) {
            htmlTemplateReply = this.getTemplateReply(commentBlock.replyes[replyBlock].replyID, commentBlock.replyes[replyBlock].nickname, commentBlock.replyes[replyBlock].preNickname, commentBlock.replyes[replyBlock].ava, commentBlock.replyes[replyBlock].replyText, commentBlock.replyes[replyBlock].date.displayDate);
